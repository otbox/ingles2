<!DOCTYPE html>

<head>
	<meta charset=UTF-8" />
	
	<title>Atividade Advanced Complete</title>
	<link rel="icon" href="https://img.a.transfermarkt.technology/portrait/header/225083-1598302373.jpg?lm=1">
	<link rel="stylesheet" type="text/css" href="styleL.css" />
</head>

<body>
<div class = 'geral'>
<a href="../New Home/index.html"><img style = "position: absolute; left: 10px; top: 10px;    " src="../44913.png" alt="" width= "50px" height=  "50px"></a>
	<div id="page-wrap">

		<h1 class ='titulo'> Advanced <p>Complete the Phrase </h1>
		<br><br><br>
		<form action="AResult.php" method="post" id="quiz">
		
            <ol>
                <div class="questao">
                    </audio><br><br>
                    <h3>I dont think they ______ have started the class yet, it's only 6:25.</h3>
                    <br>
                    <div>
                        <input type="radio" name="question-1-answers" id="question-1-answers-A" value="A" />
                        <label class = ''for="question-1-answers-A">A) Wont </label>
                    </div>
                    
                    <div>
                        <input type="radio" name="question-1-answers" id="question-1-answers-B" value="B" />
                        <label for="question-1-answers-B">B) Had</label>
                    </div>
                    
                    <div>
                        <input type="radio" name="question-1-answers" id="question-1-answers-C" value="C" />
                        <label for="question-1-answers-C">C)  Will</label>
                    </div>
                    
                    <div>
                        <input type="radio" name="question-1-answers" id="question-1-answers-D" value="D" />
                        <label for="question-1-answers-D">D) None of the above</label>
                    </div>
                </div>    

                <!-------------------------------------------------------Questao 2---------------------------------------------------------------------->
   
                <div class="questao">
                    <h3>By the time he's 25, he will have _____ from college</h3>
                    <br>
                    <div>
                        <input type="radio" name="question-2-answers" id="question-2-answers-A" value="A" />
                        <label for="question-2-answers-A">A) Graduate </label>
                    </div>
                    
                    <div>
                        <input type="radio" name="question-2-answers" id="question-2-answers-B" value="B" />
                        <label for="question-2-answers-B">B) Graduating</label>
                    </div>
                    
                    <div>
                        <input type="radio" name="question-2-answers" id="question-2-answers-C" value="C" />
                        <label for="question-2-answers-C">C) Both A and B</label>
                    </div>
                    
                    <div>
                        <input type="radio" name="question-2-answers" id="question-2-answers-D" value="D" />
                        <label for="question-2-answers-D">D) Graduated</label>
                    </div>
                </div>
                <!-------------------------------------------------------Questao 3---------------------------------------------------------------------->
                <div class="questao">
                <h3>In another five minutes, they will have _ gone for three hours</h3>
                    <br>
                    <div>
                        <input type="radio" name="question-3-answers" id="question-3-answers-A" value="A" />
                        <label for="question-3-answers-A">A) Be</label>
                    </div>
                    
                    <div>
                        <input type="radio" name="question-3-answers" id="question-3-answers-B" value="B" />
                        <label for="question-3-answers-B">B) Been</label>
                    </div>
                    
                    <div>
                        <input type="radio" name="question-3-answers" id="question-3-answers-C" value="C" />
                        <label for="question-3-answers-C">C) Being</label>
                    </div>
                    
                    <div>
                        <input type="radio" name="question-3-answers" id="question-3-answers-D" value="D" />
                        <label for="question-3-answers-D">D) All of the above</label>
                    </div>
                </div>
            </ol>
            <br><br>
            <input class="submitbtn2"style='font-size:24px' type="submit" value="Show Result"  />
		</form>
	
	</div>


</body>

</html>
